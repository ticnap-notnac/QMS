...
export async function createReport(req, res) {
  ...
  try {
    ...
    try {
      await writeAudit({ level: 'audit', source: 'ncr_reports', action: 'ncr_create', userAuthId: reportedByAuthId, details: { id: data?.id ?? null, reference_no: referenceNo } })
    } catch (err) { console.warn('Failed to record NCR create audit:', err?.message || err) }
...
export async function updateReport(req, res) {
  ...
  try {
    ...
    try {
      await writeAudit({ level: 'audit', source: 'ncr_reports', action: 'ncr_update', userAuthId: getRequestActor(req), details: { id, updates: req.body } })
    } catch (err) { console.warn('Failed to record NCR update audit:', err?.message || err) }
...
export async function updateReportInvestigation(req, res) {
  ...
  try {
    ...
    try {
      await writeAudit({ level: 'audit', source: 'ncr_reports', action: 'ncr_investigation_update', userAuthId: getRequestActor(req), details: { id, updates: req.body } })
    } catch (err) { console.warn('Failed to record NCR investigation update audit:', err?.message || err) }
    try {
      await createNotificationsForRoles({
        ...
        type: 'info', reportId: existing.id,
      })
    } catch (err) { console.warn('Failed to create investigation-ready notification:', err?.message || err) }
    return res.json(data)
  } catch (err) {
    return res.status(err.status || 500).json({ error: err.message })
  }
...
export async function deleteReport(req, res) {
  ...
  try {
    ...
    try {
      await writeAudit({ level: 'audit', source: 'ncr_reports', action: 'ncr_delete', userAuthId: getRequestActor(req), details: { id, reference_no: deleted.reference_no || null } })
    } catch (err) { console.warn('Failed to record NCR delete audit:', err?.message || err) }
...
export async function createReportSubmit(req, res) {
  ...
  try {
    ...
    try {
      ...
      await createNotificationsForRoles({ roleNames: ['admin', 'auditor'], title: `New Report Submitted: ${referenceNo}`, message: `A new NCR report ${referenceNo} has been submitted by ${reporterName}. Please review and assign it.`, type: 'info', reportId: data?.id ?? null })
    } catch (err) { console.warn('Failed to create NCR submission notification:', err?.message || err) }
  ...
  try {
    ...
    try {
      ...
      if (normalizedDecision === 'approve') {
        await createNotification({ userId: report.reported_by, title: `Report Approved: ${report.reference_no}`, message: `Your report ${report.reference_no} has been approved. It is now open for rating.`, type: 'info', reportId: report.id })
      } else {
        await createNotification({ userId: report.reported_by, title: `Report Rejected: ${report.reference_no}`, message: `Your report ${report.reference_no} was rejected. Reason: ${trimmedReason}. Please update your investigation and resubmit.`, type: 'warning', reportId: report.id })
      }
    } catch (err) { console.warn('Failed to create review notification:', err?.message || err) }

    try {
      await writeAudit({ level: 'audit', source: 'ncr_reports', action: normalizedDecision === 'approve' ? 'ncr_approve' : 'ncr_reject', userAuthId: currentUser.auth_id, details: { id: report.id, reference_no: report.reference_no, decision: normalizedDecision, reason: String(reason || '').trim() || null, status: nextStatus } })
    } catch (err) { console.warn('Failed to record NCR review audit:', err?.message || err) }
