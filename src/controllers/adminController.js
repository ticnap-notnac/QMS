...
export async function createAdminUser({
  ...
  if (!firstName || !lastName || !email || !password || !userName) {
    throw new Error('First name, last name, email, password, and username are required.')
  }
  ...
  try {
    const { data: profileData, error: profileError } = await supabase
      .from('users')
      .select('id,first_name,last_name,email,contact_number,role_id,auth_id,employee_no')
      .eq('email', email)
      .maybeSingle()
